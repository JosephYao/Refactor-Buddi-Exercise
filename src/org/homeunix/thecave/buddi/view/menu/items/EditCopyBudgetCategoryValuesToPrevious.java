...
public class EditCopyBudgetCategoryValuesToPrevious extends MossMenuItem{
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (BudgetCategory bc : ((MainFrame) getFrame()).getBudgetCategoriesInSelectedPeriod()) {
			long amount = bc.getAmountOfBudgetPeriod(currentlySelectedDate);
			try {
				if (bc.getAmountOfBudgetPeriod(bc.getBudgetPeriodType().getBudgetPeriodOffset(currentlySelectedDate, -1)) == 0)
